      ...
      <div className="flex gap-3">
        ...
        <div className="flex-1 min-w-0">
            ...
            <div className="relative" id={`post-menu-${post._id}`}> 
              ...
              {showMenu && (
                <div className="absolute right-0 mt-2 w-36 bg-[#101b2d] border border-[#23385c] rounded shadow-lg z-20 text-sm">
                  <button className="block w-full text-left px-4 py-2 hover:bg-[#17314d] text-gray-200" onClick={onEdit}>Edit</button>
                  <button className="block w-full text-left px-4 py-2 hover:bg-[#17314d] text-red-400" onClick={onDelete}>Delete</button>
                  <button className="block w-full text-left px-4 py-2 hover:bg-[#17314d] text-gray-200" onClick={onReport}>Report</button>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="mt-3 flex items-center gap-4 border-t border-[#17314d] pt-3 text-sm text-[#9fc1ff]">
            ...
            <button onClick={() => onShare?.(post)} title="Share" className="p-1.5 rounded hover:bg-[#1b2a49]">
              <FaShareAlt className="w-4 h-4" />
            </button>
            <button onClick={() => onSave?.(post._id)} title="Save" className="p-1.5 rounded hover:bg-[#1b2a49] ml-auto">
                ...
                <input
                  ...
                  onKeyDown={e => {
                    if (e.key === 'Enter' && !e.shiftKey) {
                      e.preventDefault();
                      if (onAddComment) onAddComment('submit', post._id);
                      else if (onAddComment && commentState) onAddComment(post._id);
                    }
                  }}
                  disabled={commentLoading || commentState?.loading}
                />
                <button onClick={() => (onAddComment ? onAddComment('submit', post._id) : onAddComment && commentState && onAddComment(post._id))} className="px-3 py-1 rounded bg-[#2d67b8] text-white text-sm">{commentLoading || commentState?.loading ? '...' : 'Post'}</button>
              </div>
            </div>
          )}
        </div>
